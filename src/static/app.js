document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    ...
    try {
      ...
      // Ignore stale responses so the latest action always wins.
          ...
          ? participants
              .map(
                (participant) =>
                  ...
                  `<button class="participant-delete-btn" type="button" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(participant)}" aria-label="Unregister ${escapeHtml(participant)}" title="Unregister">` +
                  `&times;` +
                  `</button>` +
                  `</li>`
              )
